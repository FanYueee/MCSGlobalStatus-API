    ...
    async (
      ...
      try {
        const cached = await queryCache.getOrLoad(
          buildDistributedCacheKey(server, type, probes),
          () => getDistributedStatus(server, type, probes),
...
export async function getDistributedStatus(
  server: string,
  type: 'java' | 'bedrock',
  probes = probeManager.getAllProbes()
): Promise<DistributedResult> {
  const { host, port } = parseAddress(server);
  const probeRegions = new Map(probes.map((probe) => [probe.id, probe.region]));
  ...
  for (const [probeId, result] of results) {
    ...
    nodes[probeId] = {
      node_region: probeRegions.get(probeId) || probeManager.getProbe(probeId)?.region || 'Unknown',
